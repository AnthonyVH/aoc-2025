 ...
 * --- Day 8: Playground ---
 *
 * Equipped with a new understanding of teleporter maintenance, you confidently step onto the
 * repaired teleporter pad.
 *
 ...
 *
 * Across the playground, a group of Elves are working on setting up an ambitious Christmas
 * decoration project. Through careful rigging, they have suspended a large number of small
 * electrical junction boxes.
 *
 * Their plan is to connect the junction boxes with long strings of lights. Most of the junction
 ...
 * every junction box. They even have a list of all of the junction boxes' positions in 3D space
 * (your puzzle input).
 *
 * For example:
 *
 ...
 * X,Y,Z coordinates. So, the first junction box in the list is at X=162, Y=817, Z=812.
 *
 ...
 * become part of the same circuit. After connecting them, there is a single circuit which contains
 * two junction boxes, and the remaining 18 junction boxes remain in their own individual circuits.
 *
 ...
 *
 * The next two junction boxes to connect are 906,360,560 and 805,96,715. After connecting them,
 ...
 * produces 40.
 *
 * Your list contains many junction boxes; connect together the 1000 pairs of junction boxes which
 * are closest together. Afterward, what do you get if you multiply together the sizes of the three
 * largest circuits?
 *
 * --- Part Two ---
 *
 ...
 *
 * Continuing the above example, the first connection which causes all of the junction boxes to form
 * a single circuit is between the junction boxes at 216,146,977 and 117,168,530. The Elves need to
 * know how far those junction boxes are from the wall so they can pick the right extension cable;
