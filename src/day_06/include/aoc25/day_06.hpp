 ...
 * the operation that needs to be performed. Problems are separated by a full column of only spaces.
 ...
 * total doesn't match the one expected by the worksheet, they realize they forgot to explain how to
 * read cephalopod math.
 *
 * Cephalopod math is written right-to-left in columns. Each number is given in its own column, with
 * the most significant digit at the top and the least significant digit at the bottom. (Problems
 * are still separated with a column consisting only of spaces, and the symbol at the bottom of the
