 ...
 * --- Day 10: Factory ---
 *
 * Just across the hall, you find a large factory. Fortunately, the Elves here have plenty of time
 * to decorate. Unfortunately, it's because the factory machines are all offline, and none of the
 * Elves can figure out the initialization procedure.
 *
 ...
 * [square brackets], one or more button wiring schematics in (parentheses), and joltage
 ...
 * and # means on. The machine has the number of indicator lights shown, but its indicator lights
 ...
 *
 * You can toggle the state of indicator lights by pushing any of the listed buttons. Each button
 * lists which indicator lights it toggles, where 0 means the first light, 1 means the second light,
 * and so on. When you push a button, each listed indicator light either turns on (if it was off) or
 * turns off (if it was on). You have to push each button an integer number of times; there's no
 * such thing as "0.5 presses" (nor can you push a button a negative number of times).
 *
 * So, a button wiring schematic like (0,3,4) means that each time you push that button, the first,
 * fourth, and fifth indicator lights would all toggle between on and off. If the indicator lights
 * were [#.....], pushing the button would change them to be [...##.] instead.
 *
 ...
 * [...#.] (0,2,3,4) (2,3) (0,4) (0,1,2) (1,2,3,4) {7,5,12,7,2}
 *
 * One way to achieve this is by pressing the last three buttons ((0,4), (0,1,2), and (1,2,3,4))
 * once each.
 *
 ...
 *
 * So, joltage requirements like {3,5,4,7} mean that the machine has four counters which are
 * initially 0 and that the goal is to simultaneously configure the first counter to be 3, the
 * second counter to be 5, the third to be 4, and the fourth to be 7.
 *
 * The button wiring schematics are still relevant: in this new joltage configuration mode, each
 * button now indicates which counters it affects, where 0 means the first counter, 1 means the
 * second counter, and so on. When you push a button, each listed counter is increased by 1.
 *
 * So, a button wiring schematic like (1,3) means that each time you push that button, the second
 * and fourth counters would each increase by 1. If the current joltage levels were {0,1,2,3},
 * pushing the button would change them to be {0,2,2,4}.
 *
 * You can push each button as many times as you like. However, your finger is getting sore from all
 * the button pushing, and so you will need to determine the fewest total presses required to
 * correctly configure each machine's joltage level counters to match the specified joltage
 * requirements.
 *
 ...
 * [.###.#] (0,1,2,3,4) (0,3,4) (0,1,2,4,5) (1,2) {10,11,11,5,10,5}
 *
 * Configuring the first machine's counters requires a minimum of 10 button presses. One way to do
 * this is by pressing (3) once, (1,3) three times, (2,3) three times, (0,2) once, and (0,1) twice.
 *
 ...
 *
 * Configuring the third machine's counters requires a minimum of 11 button presses. One way to do
 * this is by pressing (0,1,2,3,4) five times, (0,1,2,4,5) five times, and (1,2) once.
 *
 * So, the fewest button presses required to correctly configure the joltage level counters on all
 * of the machines is 10 + 12 + 11 = 33.
 *
 * Analyze each machine's joltage requirements and button wiring schematics. What is the fewest
 * button presses required to correctly configure the joltage level counters on all of the machines?
 */
