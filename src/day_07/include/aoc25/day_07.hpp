 ...
 * freely through empty space (.). However, if a tachyon beam encounters a splitter (^), the beam is
 * stopped; instead, a new tachyon beam continues from the immediate left and from the immediate
 ...
 * tachyon manifold. In this example, a tachyon beam is split a total of 21 times.
 *
 ...
 * splitting: each time a particle reaches a splitter, it's actually time itself which splits. In
 * one timeline, the particle went left, and in the other timeline, the particle went right.
 *
